...
static const UCHR K_PAIRED_CHARS[][3] = {
    "\xc7\xe7",        //"«Á",
	"()",
	"<>",
	",.",
...
static const UCHR K_Y_DEPENDANT_CHARS[] = { 'Y', 0xdd, 0x9f }; // '›'
static const UCHR *K_DEPENDANT_CHARS[] = {
	"A¿¡¬√ƒ≈",
	"E»… À",
	"IÃÕŒœ",
	"N—",
...
static UCHR GetPairedChar( UCHR inChar)
{
	UCHR outch = 0;
    int   i_inChar;
	...
	if ( (outch = (UCHR)tolower( i_inChar )) == inChar &&
		(outch = (UCHR)toupper( i_inChar )) == inChar ) 
    {
		outch = 0;
